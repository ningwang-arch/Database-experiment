...
void App::init() {
       ...
       [&](mg_http_message *hm, mg_connection *conn, OnRspCallBack cb) {
         ...
         if (filter.size() == 0) {
           ResultSet rs = ResultSet(
               conn_ptr->query("select * from houses limit " +
                               std::to_string((page - 1) * 20) + ",20"));
  ...
  POST(this->server, "/get_community",
       [&](mg_http_message *hm, mg_connection *conn, OnRspCallBack cb) {
         ...
         if (filter.size() == 0) {
           ResultSet rs = ResultSet(
               conn_ptr->query("select * from communities limit " +
                               std::to_string((page - 1) * 20) + ",20"));
  ...
  POST(this->server, "/login",
       [&](mg_http_message *hm, mg_connection *conn, OnRspCallBack cb) {
         ...
         for (std::vector<authentication_info>::iterator it =
                  this->server->v.begin();
         ...
         shared_ptr<Connection> conn_ptr = cp->getConnection();
         ResultSet rs =
             conn_ptr->query("select * from users where username='" + username +
  ...
  POST(this->server, "/register",
       [&](mg_http_message *hm, mg_connection *conn, OnRspCallBack cb) {
         ...
         root.clear();
         ResultSet rs = conn_ptr->query("select * from users where username='" +
                                        username + "';");
         if (rs.size() > 0) {
           root["status"] = AUTH_FAILED;
           root["message"] = "fail";
  ...
  POST(this->server, "/updatepwd",
       [&](mg_http_message *hm, mg_connection *conn, OnRspCallBack cb) {
         ...
         ResultSet rs =
             conn_ptr->query("select * from users where username='" + username +
                             "' and password='" + pld_pwd + "';");
         if (rs.size() > 0) {
           bool ret = conn_ptr->update("update users set password='" + new_pwd +
  ...
  POST(this->server, "/condition",
       [&](mg_http_message *hm, mg_connection *conn, OnRspCallBack cb) {
         ...
         root.clear();
         Result *r = nullptr;
         ResultSet rs = conn_ptr->query("select * from codes where belong='" +
                                        con_type + "';");
         while ((r = rs.next()) != nullptr) {
           Json::Value code;
